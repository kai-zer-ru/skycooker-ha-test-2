...
class BTLEConnection:
    ...
    async def send_auth(self):
        ...
        try:
            ...
            _LOGGER.debug("🔑 Исходный пароль: %s (тип: %s)", self._key, type(self._key))
            ...
            if isinstance(self._key, str):
                ...
                _LOGGER.debug("🔑 Пароль конвертирован из hex строки: %s", key_bytes)
            elif isinstance(self._key, list):
                # Если пароль уже список байтов, используем как есть
                key_bytes = self._key
                _LOGGER.debug("🔑 Пароль уже список байтов: %s", key_bytes)
            else:
                # В других случаях пытаемся конвертировать
                key_bytes = list(self._key)
                _LOGGER.debug("🔑 Пароль конвертирован из другого типа: %s", key_bytes)
            
            _LOGGER.debug("🔑 Финальный пароль для аутентификации: %s", key_bytes)
            
            # Проверяем, что пароль имеет правильную длину (8 байт)
            if len(key_bytes) != 8:
                _LOGGER.warning("⚠️  Неправильная длина пароля: %s (ожидается 8 байт)", len(key_bytes))
