...
async def async_setup_entry(hass: HomeAssistant, config_entry: ConfigEntry):
    ...
    try:
        ...
        _LOGGER.error("❌ Ошибка загрузки компонентов: %s", e)
        _LOGGER.exception(e)
        return False
...
class SkyCooker:
    ...
    async def sendAuth(self, conn):
        ...
        # Не ждём ответа на аутентификацию - устройство может не отвечать
        _LOGGER.info("✅ Команда аутентификации отправлена")
        self._auth = True  # Считаем аутентификацию успешной после отправки команды
        return True
